...
(define (make-new-machine)
    (let ((pc (make-register))
      ...
      (let ((the-ops
		...
		(set! labelstack '())
		'done))
;	     ((check (car insts) (break-list))
;	      (display (check-break (car insts) (break-list))))
	     (else
	      (begin
...
;;; Function to check if an instruction is within the break-list
(define (check-break instruction break-list)
  (if (member instruction break-list)
      (begin
	(if (equal? instruction (car break-list))
	    (car break-list)
	    (find instruction (cdr break-list))))
      #f))
